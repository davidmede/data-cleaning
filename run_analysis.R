...
library(tidyr)
tidy_1<-gather(database_extract, key="variable", value = "value",3:68)
tidy_2<-separate(intento,col = "variable",into = c("variable","summary","axial"),sep = "-")
tidy_3<-separate(intento_2,col = "variable",into = c("domain","variable"),sep = 1)
database_tidy<-group_by(tidy_3, activities, subject, domain, variable, summary,axial)        
summarize_database<-summarize(database_tidy, mean(value))
write.table(summarize_database, "summarize_database.txt",row.name=FALSE)
